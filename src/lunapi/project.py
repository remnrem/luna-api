...
def _coerce_var_value(value):
    """Convert Python values to Luna variable strings."""
...
class proj:
    ...
    def inst( self, n ):
        ...
        if isinstance(n, int):
            ...
            if n < 0 or n >= self.nobs():
                raise IndexError(f"index out of bounds: sample list has {self.nobs()} record(s)")

        # if the arg is a str that matches a sample-list
        if isinstance(n, str):
            sn = self.get_n(n)
            if isinstance(sn, int): n = sn
    ...
    def empty_inst( self, id, nr, rs, startdate = '01.01.00', starttime = '00.00.00' ):
        ...
        rs = int( rs )
        if nr < 0:
            raise ValueError('nr (number of records) must be a non-negative integer')
        if rs < 0:
            raise ValueError('rs (record duration in seconds) must be a non-negative integer')
    ...
    def vars(self , key=None , value=None):
        ...
        *key* and *value* are provided (or *key* is a dict), sets the
        ...
        # set a single pair
        proj.eng.opt( key, _coerce_var_value( value ) )
    ...
    def clear_vars(self,key = None ):
        ...
        if key is None:
            ...
            # and a special case: the sig list
            self.vars( 'sig', '' )
            return
