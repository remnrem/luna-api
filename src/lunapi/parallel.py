...
class ProcResult:
    ...
    def strata(self):
        ...
        if self._data is not None:
            rows = [_split_table_key(k) for k in self._data]
            return pd.DataFrame(rows, columns=["Command", "Strata"])
        if self._owner is None:
            self._file_mode_error()
    ...
    def commands(self):
        ...
        if self._data is not None:
            cmds = sorted({_split_table_key(k)[0] for k in self._data})
            return pd.DataFrame(cmds, columns=["Command"])
        if self._owner is None:
            self._file_mode_error()
        s = self._owner.strata()
    ...
    def copy(self):
        ...
        The returned ``ProcResult`` owns its data independently of the
        project cache, so subsequent ``proc()`` calls do not affect it.
        Useful when calling ``proc()`` in a loop::
        ...
        return ProcResult(
            ...
            stdout=self.stdout.copy(),
            records=self.records.copy(),
            workers=self.workers,
            _out_paths=list(self._out_paths),
            _data={k: df.copy() for k, df in self.items()},
        )
    ...
    def _parallel(self):
        """True when this result came from a parallel/batch run (records are tracked)."""
        return not self.records.empty
    ...
    def __repr__(self):
        ...
        if self._owner is None:
            ...
            if self._parallel():
                return f"ProcResult: {status} ({len(self.records)} record(s), file output: {paths})"
            return f"ProcResult: {status} (file output: {paths})"
        ...
        if self.ok:
            return f"ProcResult: completed successfully ({n_tables} table(s))"
        return f"ProcResult: completed with errors\n{self.errors.to_string(index=False)}"
    ...
    def _repr_html_(self):
        ...
        if self._owner is None:
            ...
            if self._parallel():
                return (
                    f"<p>{status}: {len(self.records)} record(s) processed.</p>"
                    f"<p>Output files:<br><code>{paths_html}</code></p>"
                )
            return f"<p>{status}.</p><p>Output files:<br><code>{paths_html}</code></p>"
        n_tables = len(self._owner_keys())
        if self._parallel():
            n_total = len(self.records)
            if self.ok:
        ...
        if self.ok:
            return f"<p><b>Completed successfully</b>: {n_tables} table(s) returned.</p>"
        return (
