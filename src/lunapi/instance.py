...
class inst:
    ...
    def refresh( self ) -> None:
        ...
        """
        self.edf.refresh()
        # reset the project-wide problem flag (problem flag is currently shared across instances)
    ...
    def refresh_channel_vars( self ) -> None:
    ...
    def clear_vars(self, keys = None ) -> None:
    ...
    def vars( self , key = None , value = None ):
        ...
        # set a single pair
        self.edf.ivar( key , _coerce_var_value(value) )
    ...
    def eval_dummy( self, cmdstr ) -> str:
    ...
    def proc( self, cmdstr ) -> ProcResult:
    ...
    def silent_proc( self, cmdstr ) -> ProcResult:
    ...
    def empty_result_set( self ) -> bool:
    ...
    def data( self, chs , annots = None , time = False ):
        ...
        if annots is not None:
            if not isinstance(annots, list): annots = [ annots ]
    ...
    def insert_signal( self, label , data , sr ) -> None:
    ...
    def update_signal( self, label , data ) -> None:
    ...
    def insert_annot( self, label , intervals, durcol2 = False ) -> None:
    ...
    def pops( self, s = None, s1 = None , s2 = None,
        ...
        if not os.path.isdir( path ):
            raise FileNotFoundError(f'POPS resource path not found: {path}')

        if s is None and s1 is None:
            raise ValueError('must set s or s1 and s2 to EEGs')
        ...
        if do_reref:
            if s is not None and m is None:
                raise ValueError('must set m to the mastoid channel when do_reref=True in single-channel mode')
            if s1 is not None and ( m1 is None or m2 is None ):
                raise ValueError('must set m1 and m2 to mastoid channels when do_reref=True in two-channel mode')
        else:
            if m is None: m = '.'
            if m1 is None: m1 = '.'
            if m2 is None: m2 = '.'
    ...
    def predict_SUN2019( self, cen , age = None , th = '3' , path = None ):
        ...
        if isinstance(cen, list) : cen = ','.join( cen )
    ...
    def hypno(self):
        ...
        if not self.has_staging():
            raise RuntimeError("no staging annotations attached")
